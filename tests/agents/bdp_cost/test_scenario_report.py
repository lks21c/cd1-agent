...
Usage:
    # 전체 테스트 실행 (34개 시나리오)
    uv run python3 -m pytest tests/agents/bdp_cost/test_scenario_report.py -v

    # 특정 그룹만 테스트
    uv run python3 -m pytest tests/agents/bdp_cost/test_scenario_report.py::TestScenarioReport::test_group_1_sudden_spike -v

    # CLI로 직접 실행
    uv run python3 -c "import sys; sys.path.insert(0, '.'); from tests.agents.bdp_cost.test_scenario_report import run_scenario_report; run_scenario_report()"

    # LocalStack 통합 테스트
    AWS_ACCESS_KEY_ID=test AWS_SECRET_ACCESS_KEY=test AWS_DEFAULT_REGION=ap-northeast-2 \\
      uv run python3 -m pytest tests/agents/bdp_cost/test_scenario_report.py -v -m localstack
"""
...
from src.agents.bdp_cost.services.anomaly_detector import CostDriftDetector, Severity
